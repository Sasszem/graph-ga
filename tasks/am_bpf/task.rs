...
pub fn get_fitness(ckt: &Circuit) -> f64 {
    ...
    score += d.iter().map(|(f, r)| {
        ...
        } else if &525000.0 < f && f < &1606500.0 {
            let d = (mag + 6.0).abs();
            if d < 0.5 {
                d*100.0
            } else {
...
fn main() {
    ...
    eprintln!("Staring GA task!");
    galib::ga::do_ga(&ckt, 500, 1000, get_fitness, print_result, mutate, galib::ga::crossover_random_swap, "am_bpf_fitness.csv", "am_bpf_checkpoint.csv");
}